...
const deployFunction: DeployFunction = async function ({
  ...
  if ((await misoTokenFactory.accessControls()) === ethers.constants.AddressZero) {
    console.log('MISOTokenFactory initilising')
    await (await misoTokenFactory.initMISOTokenFactory(accessControls.address)).wait()
  ...
  if (templateId.toNumber() === 0) {
    ...
    await (await misoTokenFactory.addTokenTemplate(mintableToken.address)).wait()
...
deployFunction.dependencies = ['MISOAccessControls', 'FixedToken', 'MintableToken', 'SushiToken']
